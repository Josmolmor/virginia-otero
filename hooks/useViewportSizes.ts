...
function useViewportSizes(input) {
  const hasher =
    (typeof input == 'function' ? input : input?.hasher) || undefined;

  const debounceTimeout =
    typeof input?.debounceTimeout == 'number' ? input?.debounceTimeout : 0;

  const throttleTimeout =
    (typeof input == 'number' ? input : input?.throttleTimeout) || 0;
  ...
  const returnValue = useMemo(() => {
    switch (dimension) {
      default:
      case 'both': {
        return [state?.vpW || 0, state?.vpH || 0, onResize];
      }
      case 'w': {
        return [state?.vpW || 0, onResize];
      }
      case 'h': {
        return [state?.vpH || 0, onResize];
      }
    }
